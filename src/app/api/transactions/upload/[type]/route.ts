...
export async function POST(
  ...
  try {
    ...
    const keywords = [
      ...new Set(transactions.map((record) => serializer(record).keyword)),
    ];
    const keywordLabels = await Promise.all(
      keywords.map((keyword) => findOrCreateKeyword(keyword)),
    );
    const keywordMap = new Map(
      keywordLabels.map((kl) => [kl.name, kl.labelId]),
    );
